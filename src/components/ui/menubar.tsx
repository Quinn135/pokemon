...
function Menubar({
  ...
  return (
    <MenubarPrimitive.Root
      data-slot="menubar"
      className={cn("bg-background h-9 rounded-lg border flex items-center", className)}
      {...props}
    />
  )
}
...
function MenubarTrigger({
  ...
  return (
    <MenubarPrimitive.Trigger
      ...
      className={cn(
        "hover:bg-muted aria-expanded:bg-muted px-3 h-full text-xs/relaxed font-medium flex items-center outline-hidden select-none",
  ...
  return (
    <MenubarPortal>
      <MenubarPrimitive.Content
        ...
        className={cn("bg-popover text-popover-foreground data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 ring-foreground/10 min-w-32 rounded-lg rounded-t-none p-1 shadow-md ring-1 duration-100 z-50 origin-(--radix-menubar-content-transform-origin) overflow-hidden", className)}
        {...props}
      />
    </MenubarPortal>
  )
